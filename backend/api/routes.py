...
import smtplib # Mocking for now
...
def mock_send_email(name: str, email: str, message: str):
    # This is a mock function that simulates sending an email immediately
...
async def contact_endpoint(request: ContactRequest, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    # Send email immediately (mock)
    mock_send_email(request.name, request.email, request.message)
    
    # Save to SQLite database as backup
...
async def get_projects():
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            # Recruiter-friendly custom content
            CUSTOM_CONTENT = {
                "job-hunter": {
            ...
            # Filter for only the target repos
            target_repo_data = {}
            for r in repos:
            ...
            for repo_id in TARGET_REPOS:
                ...
                if r:
                    # Use custom tech stack if available, else fallback to GitHub's primary language
                    ...
                    # Fallback if repo not found on GitHub or is private
            ...
            # Fallback to local yaml if API fails
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code == 200:
                ...
                commits = sum(r.get("size", 0) for r in repos) # approximation
